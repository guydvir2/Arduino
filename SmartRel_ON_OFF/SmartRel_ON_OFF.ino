...
const char *ver = "WEMOS_WDT_OTA_2switches_2.82";
//###################################################
...
// TimeOut Constants
int maxTO = 100; //minutes
int timeIncrements = 10; //minutes
...
void allOff() {
  digitalWrite(output_1Pin, !relayON); //relay off
  digitalWrite(output_2Pin, !relayON);
}
...
void checkSwitch_1() {
  if (digitalRead(input_1Pin) == buttonPressed) {
    ...
    if (digitalRead(input_1Pin) == buttonPressed && millis() - pressTO_input1 > delayBetweenPress) {
      ...
      else if (timeInc_counter <= maxTO / timeIncrements - 1 && millis() - pressTO_input1 < 3000 ) {
...
void switch_1_terminator() {
  ...
  else if ( endTime != 0 && endTime > millis() ) {
