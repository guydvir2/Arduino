...
int deBounceInt = 50;
int time_NOref_OLED = 5; // seconds to stop refresh OLED
...
void checkSwitch_Pressed(byte i)
{
        ...
        {
                delay(deBounceInt);
                if (digitalRead(inputs[i]) == SwitchOn && still_pressed == false)
                {
                        while (digitalRead(inputs[i]) == SwitchOn) // count press duration
                        {
                                if (timeCouter == 0)
                                {
                                        timeCouter = millis();
                        ...
                        else if (timeInc_counter == 0 && last_relState[i] == !RelayOn)
                        { // first press turns on - still not in TO mode
                                switchIt("Button", i, 1, "", false);
                                TO[i]->updateStart(now()); // register when started in FS
                                timeInc_counter += 1;
                        }
                        ...
                        { // additional presses update timer countdown
                                int newTO = TO[i]->remain() + 1 * timeIncrements * 60; // add a timeQoute to on going TO
...
void FailSafe_looper(byte i, float timeout_val)
{ // timeout_val in minutes - to force shut down
...
void recoverReset()
{
        ...
        { // before getting online/offline MQTT state
                ...
                for (int i = 0; i < NUM_SWITCHES; i++)
                {
                        if (hReset_eeprom.hBoot && HARD_REBOOT)
                        { // using HardReboot
                                TO[i]->restart_to();
                                iot.pub_log("--> ForcedBoot. Restarting TimeOUT");
                        }
                        else if (rebootState == 0 && ON_AT_BOOT == true)
