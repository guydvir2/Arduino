...
void MQTT_to_telegramBOTS()
{
        ...
        static long MSG_clk = 0;
        static long LOG_clk = 0;
        ...
        if (iot.extTopic_newmsg_flag)
        {
                ...
                else if (strcmp(incoming_mqtt.from_topic, MSG_TOPIC) == 0)
                {
                        MSG_clk = millis();
                        MSG_buf += String('\n') + String(incoming_mqtt.msg) + String('\n');
                }
        ...
        if (LOG_clk != 0 && (millis() - LOG_clk) > timeout || LOG_buf.length() > bufsize)
        {
#if USE_LOG_BOT
        ...
        if (MSG_clk != 0 && (millis() - MSG_clk) > timeout || MSG_buf.length() > bufsize)
        {
#if USE_MSG_BOT
...
void startTelegramBOTS()
{
        ...
        LISTEN_TO_TELEGRAM.chatID = CHAT_ID;
#endif
        char clk[25];
        char msg[50];
