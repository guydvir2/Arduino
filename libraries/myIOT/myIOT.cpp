...
void myIOT::looper(){
        if (!noNetwork_flag ) { // if boot start without wifi - only a timeout reset will start with flag is false
        ...
        if (noNetwork_Clock>0) {  // no Wifi or no MQTT will cause a reset
