...
void myIOT::looper() {
	...
	if (useOTA) { // checks for OTA
		acceptOTA();
	}
	if (noNetwork_Clock > 0 && useNetworkReset) { // no Wifi or no MQTT will cause a reset
...
bool myIOT::startWifi(char *ssid, char *password) {
	...
	while (WiFi.status() != WL_CONNECTED
			&& millis() - startWifiConnection < WIFItimeOut) {
		delay(500);
		if (useSerial) {
			Serial.print(".");
		}
...
void myIOT::callback(char *topic, byte *payload, unsigned int length) {
	...
	if (strcmp(incoming_msg, "boot") == 0) {
		...
		sprintf(msg,
				"ver: IOTlib: [%s], WDT: [%d], OTA: [%d], SERIAL: [%d], ResetKeeper[%d], FailNTP[%d], debug_Log[%d]",
				ver, useWDT, useOTA, useSerial, useResetKeeper, resetFailNTP,
				useDebug);
		pub_msg(msg);
...
// ############################### FVars CLASS #################################
FVars::FVars(char *key, char *pref, char *fname) :
		json(fname, true) {
...
timeOUT::timeOUT(char *sw_num, int def_val) :
		endTimeOUT_inFlash(_key1, sw_num), inCodeTimeOUT_inFlash(_key2, sw_num), updatedTimeOUT_inFlash(
		_key3, sw_num), startTimeOUT_inFlash(_key4, sw_num), dailyTO_inFlash(
				"TO.json", true) {
...
bool timeOUT::begin() {
	...
	//         }
	// }
	else if (endTO_inFlash == 0) { // timeour ended correctly
		return 0;
	}
	sprintf(dTO_pubMsg, "");
}
...
void timeOUT::convert_epoch2clock(long t1, long t2, char *time_str,
	...
	minutes = (int) ((time_delta - days * sec2days - hours * sec2hours)
			/ sec2minutes);
	seconds = (int) (time_delta - days * sec2days - hours * sec2hours
			- minutes * sec2minutes);
...
void timeOUT::dailyTO_looper(dTO &dailyTO) {
	...
	if (dailyTO.flag == true) {
		if (dailyTO.onNow == false) { // start
			if (hour(t) == dailyTO.on[0] && minute(t) == dailyTO.on[1]
					&& second(t) == dailyTO.on[2]) {
...
void timeOUT::restart_dailyTO(dTO &dailyTO) {
	time_t t = now();
	dTO temp_dTO = { { hour(t), minute(t), second(t) }, { dailyTO.off[0],
			dailyTO.off[1], dailyTO.off[2] }, 1, 1, 0 };
...
void mySwitch::_recoverReset(int rebootState) {
	if (rebootState == 0 && onAt_boot) { // PowserialerOn - not a quickReboot
		if (usePWM) {
	...
	} else if (hReboot.resetFlag && useHardReboot) { // using HardReboot
		if (usePWM) {
			switchIt("onAtBoot", def_power);
		} else {
			switchIt("onAtBoot", RelayOn);
	...
	} else if (TOswitch.looper() == 0) { // was not during TO
		if (rebootState == 1) {
	...
	if (momentary) {
		...
		if (digitalRead(swPin) != inputState) {
			...
			if (digitalRead(swPin) != inputState) {
				...
				if (inputState == SwitchOn && digitalRead(_switchPin) != RelayOn) { // turn in TO
					TOswitch.restart_to();
				} else if (inputState != SwitchOn) { // turn off
					if (TOswitch.remain() > 0) { // turn off when in TO
						TOswitch.endNow();
					} else if (digitalRead(_switchPin) == RelayOn
							&& TOswitch.remain() == 0) { // turn off when only ON
						switchIt("Button:", 0);
					}
				}
			}
...
void mySwitch::_safetyOff() {
	if (_safetyOff_clock != 0
			&& millis() - _safetyOff_clock > set_safetyoff * 60 * 1000L) {
...
void mySwitch::getMQTT(char *parm1, int p2, int p3, int p4) {
	...
	if (strcmp(parm1, "offline") != 0 && strcmp(parm1, "online") != 0
			&& strcmp(parm1, "resetKeeper") != 0) {
...
void mySwitch::_TOlooper(int det_reset) {
	if (det_reset != 2) {
		...
		if (relayState != last_relayState) { // change state (ON <-->OFF)
