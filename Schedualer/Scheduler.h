...
bool useIndicLED[MAX_NUM_SCHEDS] = {false, false}; /* use indication leds when ON*/

bool indic_ON[MAX_NUM_SCHEDS] = {false, false};
bool output_ON[MAX_NUM_SCHEDS] = {LOW, HIGH};   /* OUTPUT when ON is HIGH or LOW */
...
bool outputPWM[MAX_NUM_SCHEDS] = {false, false};

uint8_t trigType[MAX_NUM_SCHEDS] = {0, 0};          /* Input type */
uint8_t inputPin[MAX_NUM_SCHEDS] = {D6, D5};        /* IO for inputs */
uint8_t outputPin[MAX_NUM_SCHEDS] = {2, 4};         /* IO for outputs */
uint8_t indicPin[MAX_NUM_SCHEDS] = {4, 2};          /* IO for idication LEDS */
...
char sw_names[MAX_NUM_SCHEDS][20] = {"SW1", "SW2"}; /* Name of each Switch, as shown on MQTT msg */
